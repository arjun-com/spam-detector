...
class SpamDetectionModel:
    def __init__(self, CSV_PATH):
        df = pd.read_csv(
            CSV_PATH, sep="\t", header=None, names=["label_str", "text"], quoting=3
        )
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            self.X, self.y, test_size=0.2, random_state=100, stratify=self.y
        )
                ...
                (
                    ...
                    TfidfVectorizer(
                        ...
                        ngram_range=(
                            1,
                            2,
                        ),  # Checks for unigram ( one word tokens like 'free' ) and bigram ( two word token 'shop now' )
        ...
        prec, recall, f1, _ = precision_recall_fscore_support(
            self.y_test, self.y_pred, average="binary"
        )
